...
class TestRemotePmacInterface(unittest.TestCase):
    def setUp(self):
        self.obj = dls_pmaclib.dls_pmacremote.RemotePmacInterface()

    def test_init(self):
        assert self.obj.verboseMode == False
        assert self.obj.hostname == ""
        assert self.obj.port == None
        assert self.obj.parent == None
        assert self.obj.isConnectionOpen == False
        assert self.obj.timeout == 3.0
        assert self.obj._isModelGeobrick == None
        assert self.obj._pmacModelCode == None
        assert self.obj._pmacModelName == None
        assert self.obj._shortModelName == None
        assert self.obj._numAxes == None
        assert self.obj.baud_rate == None
        assert self.obj.comm_port == None
        assert self.obj.last_received_packet == None
        assert self.obj.last_comm_time == None
        assert self.obj.n_timeouts == None
        assert self.obj.serial == None
    ...
    def test_is_model_geobrick(self, mock_get_code):
        mock_get_code.return_value = 603382
        assert self.obj.isModelGeobrick() == True
    ...
    def test_axis_in_range(self, mock_get_axes):
        ...
        assert self.obj.checkAxisIsInRange(2) == None
    ...
    def test_isMacroStationAxis_not_geobrick(self, mock_checkaxis, mock_isgeobrick):
        mock_checkaxis.return_value = None
        mock_isgeobrick.return_value = False
        assert self.obj.isMacroStationAxis(1) == True
        assert mock_checkaxis.called
        assert mock_isgeobrick.called
    ...
    def test_isMacroStationAxis_false(self, mock_checkaxis, mock_isgeobrick):
        mock_checkaxis.return_value = None
        mock_isgeobrick.return_value = True
        assert self.obj.isMacroStationAxis(1) == False
        assert mock_checkaxis.called
        assert mock_isgeobrick.called
    ...
    def test_isMacroStationAxis_true(self, mock_checkaxis, mock_isgeobrick):
        mock_checkaxis.return_value = None
        mock_isgeobrick.return_value = True
        assert self.obj.isMacroStationAxis(10) == True
        assert mock_checkaxis.called
        assert mock_isgeobrick.called
    ...
    def test_setVar_successful(self, mock_sendcmd):
        ...
        assert self.obj.setVar("var", 100) == None
    ...
    def test_setAxisSetupIVar(self, mock_checkaxis, mock_setvar):
        ...
        assert self.obj.setAxisSetupIVar(1, 1, 5) == None
        mock_checkaxis.assert_called_with(1)
    ...
    def test_setAxisMsIVar(self, mock_check, mock_get, mock_set):
        ...
        assert self.obj.setAxisMsIVar(1, 100, 5) == None
    ...
    def test_setOnboardAxisI7000PlusIVar(self, mock_get, mock_setvar):
        mock_get.return_value = 100
        mock_setvar.return_value = None
        assert self.obj.setOnboardAxisI7000PlusIVar(1, 2, 5) == None
        mock_get.assert_called_with(1)
